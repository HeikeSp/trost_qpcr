Prediction Model using qPCR data (all)
========================================================
...
```{r load data}
tolerance <- read.table("../../../Prediction_Model/output/tolerance_phenotyper_metadata_2sub.txt", 
                                                 sep="\t", header=TRUE)
...
# train/test data
train_data <- subset(log_norm_ct, samplelist_joined$model_set=="train")
test_data <- subset(log_norm_ct, samplelist_joined$model_set=="test")

# train/test info
train_info <- subset(samplelist_joined, samplelist_joined$model_set=="train")
test_info <- subset(samplelist_joined, samplelist_joined$model_set=="test")
```
...
all_data <- subset(log_norm_ct, !samplelist_joined$model_set=="NA")
all_info <- subset (samplelist_joined, !samplelist_joined$model_set=="NA")

# all control samples (ohne NA -> 3 Kutlivare)
all_data_control <- subset(log_norm_ct, samplelist_joined$treatment=="control" & !samplelist_joined$model_set=="NA")
all_info_control <- subset (samplelist_joined, samplelist_joined$treatment=="control" & !samplelist_joined$model_set=="NA")
```
...
pdf("../figures/all_tree_train_pruned.pdf", width=13, height=8)
...
pdf("../figures/all_tree_train_control_pruned.pdf", width=13, height=8)
...
pdf("../figures/all_tree_all_pruned.pdf", width=13, height=8)
...
pdf("../figures/all_tree_all_control_pruned.pdf", width=13, height=8)
...
pdf("../figures/all_tree_cat2_train_pruned.pdf", width=13, height=8)
...
pdf("../figures/all_tree_cat3_train_pruned.pdf", width=13, height=8)
...
pdf("../figures/all_tree_cat3_all_pruned.pdf", width=13, height=8)
